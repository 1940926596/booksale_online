<template>
  <div class="page"
    ...
    <div class="middle">
      <div class="content1">收到报价 | 交易号: {{ transactionId }}</div>
      <div class="content2">图书信息 | {{ book[0].bookId }} | {{ book[0].bookName }} | {{ book[0].bookTypes }}</div>
      <div class="content3">求购者信息 | {{ buyUser[0].name }} | {{ buyUser[0].email }} | {{ buyUser[0].position }}</div>
      <div class="content4">报价时间: {{ time }}</div>
    </div>
    <div class="right">
      <div class="label1" style="border: 1px solid black;padding: 10px;cursor: pointer" @click="complete">完成交易</div>
  ...
  watch: {
    transactionId: function (val) {
      ...
      this.axios.get(this.$store.state.host + "bookOneList?bookId=" + this.bookId).then((res) => {
        ...
        this.axios.get(this.$store.state.host + "sqlOneList?user_id=" + this.buyUserId).then((res) => {
          this.buyUser = res.data
          console.log(this.buyUser)
        })
      })
    }
  },
